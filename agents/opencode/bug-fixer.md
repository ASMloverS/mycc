...
Invoke `skill({ name: "systematic-debugging" })` immediately.
...
Root cause found → invoke `skill({ name: "doc-write" })`.
...
Invoke `skill({ name: "test-driven-development" })`.
...
Grep `BUG-FIXER-DEBUGGING` → remove all hits.
Rebuild/retest → confirm clean.

## Step 5: Simplify

Invoke `skill({ name: "code-simplifier" })` on modified files only.

## Step 6: Review

Phase A — self: check correctness, edge cases, logic gaps.
Phase B — dispatch `code-reviewer` subagent via Task tool:
