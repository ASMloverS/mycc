...
Invoke `skill({ name: "doc-write" })`. Input: prompt context + ref doc content.
...
Invoke `skill({ name: "doc-refine" })` — ultra compress design doc.
...
All task docs → invoke `skill({ name: "doc-refine" })` ultra compress.
