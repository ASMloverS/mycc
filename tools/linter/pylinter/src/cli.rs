...
#[derive(Parser, Debug)]
#[command(name = "pylinter", version, about = "Python 3.14+ linter: format + style check + static analysis")]
pub struct Args {
    #[arg(required = true)]
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !args.format_only {
        ...
        print_new_diags(&all_diags, &mut seen, args.verbose, args.quiet);
    ...
    if !args.format_only && config.analysis.level != AnalysisLevel::None {
        ...
        print_new_diags(&all_analysis_diags, &mut seen, args.verbose, args.quiet);
...
fn print_new_diags(
    diags: &[crate::common::diag::Diagnostic],
...
fn build_ignore_matcher(args: &Args) -> crate::ignore::IgnoreMatcher {
    ...
    let ignore_path = std::path::Path::new(".pylinterignore");
