...
use vcs_blame::reporter::Reporter;
...
fn test_help_output() {
    ...
    assert!(stdout.contains("vcs-blame"));
...
fn test_python_traceback_parser() {
    ...
    let frames = vcs_blame::parser::traceback_py::parse_python_traceback(&input);
...
fn test_cpp_stacktrace_parser() {
    let input = std::fs::read_to_string("tests/fixtures/traceback/sample_cpp.txt")
        .expect("fixture not found");
    let frames = vcs_blame::parser::traceback_cpp::parse_cpp_stacktrace(&input);
    assert_eq!(frames.len(), 3);
...
fn test_diff_parser() {
    let input = std::fs::read_to_string("tests/fixtures/diff/sample.diff")
        .expect("fixture not found");
    let diffs = vcs_blame::parser::diff::parse_unified_diff(&input).unwrap();
...
fn test_blame_aggregation() {
    ...
    let entries = vec![
        vcs_blame::blame::BlameEntry {
            ...
            author_time: chrono::NaiveDate::from_ymd_opt(2024, 1, 1)
                ...
                .and_hms_opt(0, 0, 0)
                .unwrap(),
            vcs: vcs_blame::blame::VcsKind::Git,
    ...
    let result = vcs_blame::blame::aggregate(entries, &HashMap::new());
...
fn test_text_reporter() {
    let result = vcs_blame::blame::BlameResult {
        entries: vec![vcs_blame::blame::BlameEntry {
            file: "test.py".into(),
            line: 10,
            author: "alice".into(),
            author_mail: "a@a.com".into(),
    ...
    let reporter = vcs_blame::reporter::text::TextReporter { no_color: true };
    let mut buf = Vec::new();
...
fn test_json_reporter() {
    ...
    let reporter = vcs_blame::reporter::json::JsonReporter;
    let mut buf = Vec::new();
    reporter.render(&result, &mut buf).unwrap();
    let output = String::from_utf8(buf).unwrap();
