...
impl Config {
    ...
    pub fn resolve_vcs(&self, cli: Option<&str>) -> Option<String> {
        ...
        if let Ok(val) = std::env::var("VCS_BLAME_VCS") {
            return Some(val);
        }
        self.vcs.clone()
    }
    ...
    pub fn resolve_format(&self, cli: Option<&str>) -> String {
        ...
        if let Ok(val) = std::env::var("VCS_BLAME_FORMAT") {
            return val;
        }
    ...
    pub fn resolve_no_color(&self, cli_flag: bool) -> bool {
        ...
        if let Ok(val) = std::env::var("VCS_BLAME_NO_COLOR") {
    ...
    pub fn resolve_output(&self, cli: Option<&str>) -> Option<String> {
        ...
        if let Ok(val) = std::env::var("VCS_BLAME_OUTPUT") {
            return Some(val);
        }
        self.output.clone()
    }
    ...
    pub fn resolve_config_path(cli: Option<&str>) -> std::path::PathBuf {
        ...
        std::path::PathBuf::from(".vcs-blame.yaml")
    }
}
...
mod tests {
    ...
    fn test_load_missing_config() {
        let cfg = Config::load(Path::new("/nonexistent/.vcs-blame.yaml")).unwrap();
