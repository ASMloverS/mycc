---
name: code-simplify
description: Parallel code review and cleanup of recent changes. Spawns three focused reviewer subagents (reuse, quality, efficiency) in parallel, then sequentially applies the findings. Use after writing or editing code to improve clarity without changing behavior. Trigger whenever the user asks to simplify, clean up, polish, refactor for readability, review recent edits, or improve code quality — even if they don't use the word "simplify".
argument-hint: [optional focus, e.g. "memory efficiency"]
---

# code-simplify — parallel review, sequential apply
...
> - **Respect house conventions.** Project conventions (CLAUDE.md) override generic idioms.
>
...
> - Stringly-typed code: raw strings where constants, enums, or typed identifiers already exist
...
> - Unnecessary wrapper nesting: container elements or classes that add no behavior or structural value — check if the inner element's own properties already provide the needed layout or grouping
...
- **Meaningfully different `new_string`** (different algorithm, different control flow, different intermediates) → check the **Principles** against each variant:
  ...
  Never stop to ask. Surfacing a style call to the user mid-run defeats the point of having principles. The user invoked `/code-simplify` to clean code, not to answer a quiz.
...
path/to/file.ext
  ...
  ✓ [Reuse]      replaced manual loop with existing utility on line 53
