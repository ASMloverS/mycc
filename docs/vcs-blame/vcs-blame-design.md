...
```
vcs-blame [GLOBAL OPTIONS] <COMMAND>
...
Global Options:
  ...
  --config <PATH>               Config file (default: .vcs-blame.yaml)
...
```
vcs-blame blame <FILE>[:LINE[-ENDLINE]] [OPTIONS]
...
No subcommand defaults to blame: `vcs-blame foo.py:10` == `vcs-blame blame foo.py:10`.
...
```
vcs-blame traceback [TEXT | -f FILE | --stdin]
...
Env var mapping: `VCS_BLAME_VCS`, `VCS_BLAME_FORMAT`, `VCS_BLAME_NO_COLOR`, `VCS_BLAME_OUTPUT`.
