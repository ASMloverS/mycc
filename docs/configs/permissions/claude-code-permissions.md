...
"deny": [
  // 1) 强制/可丢工作的 git 操作
  // 注：「标志后置」变体（git push origin main --force）因中段 * 无官方保证，改为枚举常见顺序
  "Bash(git push --force*)",
...
"allow": [
  ...
  // 文件/文本工具（chmod / ln 改为默认 ask；高频具体形式按需加 settings.local.json）
...
| `git push origin main --force` | 标志后置；中段 `*` 已移除，此变体不被 deny 覆盖（已知不拦）|
...
- `git push origin main --force`（标志后置）—— 因中段 `*` 无官方保证已移除，如需拦截需 PreToolUse hook
...
- `--diff` (bool)：打印将新增的条目逐行；与 `--apply` 同时使用时先打印 diff，再写盘，末尾打印 backup 路径
...
- **不读环境变量**；推荐集从同目录 `tools/perms-update.recipe.json` 加载（标准库 `json.load`）
...
4. **allow 通畅性（回归）**：
   - `git status` / `git log --oneline` / `cargo build` / `python -c "print(1)"` / `cmake --build build` / `npm test` / `pytest -q` → 静默通过（无误拦）
...
7. **自动化回归（可选）**：新增 `tools/test_perms_update.py`，参照 `tools/test_sync_config.py`，覆盖：
