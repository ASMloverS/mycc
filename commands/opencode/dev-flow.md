---
allowed-tools: Read, Task, Edit, Glob, Grep
...
Spawn `Task(subagent_type="code-implementer")`:
...
If implement step reports failure (tests not passing) → abort, print failure reason.

## Step 2 — Review

Spawn `Task(subagent_type="code-reviewer")`:
...
- CRITICAL/MAJOR found + iteration < 2:
  1. Spawn `Task(subagent_type="code-implementer")`:
     ```
     Original spec:
     <spec>
     ...
     ```
  2. Spawn `Task(subagent_type="code-reviewer")` again w/ same spec + same impl files
  3. Increment iteration. Repeat check.
- CRITICAL/MAJOR found + iteration = 2 → keep changes, report remaining issues in summary.
